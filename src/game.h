...
typedef struct __attribute__((packed)) {
    ...
    uint8_t  paddle_max;                /* 0x2d3f 172, and it moves as the paddle grows: morph_step adds a width delta to paddle_width and subtracts the same from this (1ac2:34f0 and 1ac2:34f6), so the two always sum to 199, the playfield's right edge. 172 is that less the 27 paddle_sets[0] is wide, the same way WALL_RIGHT is that less the ball's. Was also PADDLE_HIGH */
