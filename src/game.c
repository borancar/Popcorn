 ...
 *
 *     x_offset / y_offset = [+0x17] / [+22]
 *
 * so +0x17 is the horizontal component of the slope and +22 the vertical -
 * the pair is stored (dy, dx), which is the opposite of how it reads.  The
 * accumulators at +26/+0x1b count *steps along the major axis* away from
 * the anchor at +24/+0x19, which is where the segment began; the live
 * position written to +0x00/+0x01 is anchor plus offset.  So +24/+0x19 is
 * not where the ball is, and stays put until the next bounce.
 */
void ball_step(ball_t *b)
{
    uint8_t dy = b->dy, dx = b->dx;
    uint8_t off_x, off_y;
...
/* The two indicator columns down the panel, both from the same scan line:
 * the safety net's on the left and the M capsule's on the right. Each is
 * filled when the bonus arrives and drained a cell a frame after that. */
/* Where a capsule stops wandering and starts following the homing script:
 * `cmp al,0x78` at 1ac2:3d3e, and the branch it takes ends with `inc al`
 * at 1ac2:3d90 - so the script's own base row is one lower. */

void input_keyboard(void)
{
    if (--global.repeat_count != 0)
        return;
    if (global.repeat_div != 1)
        global.repeat_div--;
    global.repeat_count = global.repeat_div;

    int32_t go_left = global.key_left != 0;
    if (global.key_left == global.key_right) {
        if (!global.key_left) {              /* neither key held */
            global.repeat_count = REPEAT_RESET;
            global.repeat_div = REPEAT_RESET;
            if (global.paddle_x > global.paddle_max)
                global.paddle_x = global.paddle_max;
            return;
        }
        go_left = global.last_dir == 0;      /* both: the most recent wins */
    }
    ...
    if (!go_left) {
        uint8_t x = global.paddle_x + 1;
        if (x > global.paddle_max)
            x = global.paddle_max;
        global.paddle_x = x;
    } else {
        uint8_t x = global.paddle_x - 1;
        if (x < global.paddle_min)
            x = global.paddle_min;
        global.paddle_x = x;
    }
}
 ...
 */
void input_mouse(uint16_t mouse_x, uint16_t buttons)
{
 ...
 *
 * Both halves of the CGA aperture to and from a 16,000-byte buffer at image
 * 0x10250, which the original reaches as 0xc46:0x3df0 - one of the 35
 * relocated segment constants, and past what DS can address.
 *
 * That buffer **is** the intro's artwork: assets.screen_save unions scroll_rows,
 * reveal and logo, which this overwrites. save_screen is called once, the
 * moment the intro finishes, and restore_screen is how the menu comes back
 * every time after - so the pictures are spent by the time the screen lands
 * on them.
 */
...
{
    memcpy(assets.screen_save[0], g_vram, SCREEN_HALF);
    memcpy(assets.screen_save[1], g_vram + CGA_PLANE, SCREEN_HALF);
}

void restore_screen(void)
{
    memcpy(g_vram, assets.screen_save[0], SCREEN_HALF);
    memcpy(g_vram + CGA_PLANE, assets.screen_save[1], SCREEN_HALF);
}
...
/* The paddle's top row on screen. Everything about where the paddle sits is
 * measured from here - it is seven rows tall, so it ends on PADDLE_BOTTOM,
 * and the band the ball is caught in starts three above it, the same three
 * that `paddle_x - 3` puts on the left. */
 ...
 * so scan line 184. That whole expression is cga_at(x, PADDLE_Y): 184 is even
 * so there is no plane offset, and 92 * CGA_STRIDE is the 0x1cc0. The
 * alternating `+0x2000` / `-0x1fb0` after it is the interlace step, the same
 * one cga_next_row() spells out.
 */
void paddle_row_offsets(uint8_t x, paddle_rows_t *rows)
{
    uint16_t off = cga_at(x, PADDLE_Y);
...
void draw_paddle(const uint8_t *sprite)
{
    ...
    uint16_t x = global.paddle_x;
    global.paddle_prev_x = x;
    paddle_row_offsets(x, &global.paddle_rows[0]);

    /* Pick the copy pre-shifted to this pixel within its byte. */
    memcpy(global.paddle_pix[0],
           sprite + (x & 3) * PADDLE_IMAGE, PADDLE_IMAGE + 1);

    blit_xor(global.paddle_pix[1], &global.paddle_rows[1]);   /* erase where it was */
    blit_xor(global.paddle_pix[0], &global.paddle_rows[0]);     /* draw where it is */
}

/* The glyph table's own order: 0 space, 1 to 10 the digits, 11 the dash, 12
 * to 37 the letters, 38 the colon and 39 the text-entry cursor. That is why
 * a digit is `c - 47` and a letter `c - 53` - '0' lands on 1 and 'A' on 12. */
...
static uint8_t glyph_of(char c)
{
    ...
    uint8_t u = c;
    if (u == ':')  return 38;
    if (u == 0xff) return 39;           /* the text-entry cursor */
    if (u == '-')  return 11;
    if (u >= '0' && u <= '9') return u - 47;
    if (u >= 'A' && u <= 'Z') return u - 53;
 ...
 * global.font holds 24 bytes per glyph: twelve rows of one word,
...
{
    uint8_t req = runtime.sound_request;
    ...
    const note_t *note = (const note_t *)runtime_ptr(runtime.sound_ptr);
    if (note->pitch == 0 && note->hold == 0) {  /* end of tune */
        speaker_off();
        return;
    }
    runtime.sound_ptr += 2;
    runtime.sound_timer = note->hold;
    io_sound((note->pitch << 8) | 1);
}
...
void read_speed_setting(uint8_t speed)
{
    ...
    if (speed == 0)
        speed = 111;               /* the readme's 110, before the decrement */
    runtime.delay_count = speed - 1;
}
...
/* Where the bar starts: `mov di, 0x213f` at 1ac2:0793, which is the
 * right-hand byte column - four pixels, 316 to 319 - on scan line 7. It
 * sweeps left from there, 26 columns of four pixels. */

/* 1ac2:078b  intro_curtain
 *
 * Two things, in one routine. The first sweeps a dithered bar across the
 * screen: 26 columns, four phases each, and in every phase `ah` gains two
...
void intro_curtain(void)
{
    ...
    uint16_t di0 = cga_at(CURTAIN_X, CURTAIN_Y);
    ...
    for (uint16_t bx = 26; bx > 0; bx--, di0--) {
        ...
        for (uint16_t dh = 4; dh > 0; dh--) {
            ...
            if (bx == 1) {
                ...
                dl = dl << 2;
                al = 0x55 & dl;
                di = cga_at(CURTAIN_X, CURTAIN_Y);
                for (uint16_t cx = 49; cx > 0; cx--, di += CGA_STRIDE)
                    g_vram[di & (CGA_SIZE - 1)] = al;
                di = cga_next_row(cga_at(CURTAIN_X, CURTAIN_Y));
                for (uint16_t cx = 49; cx > 0; cx--, di += CGA_STRIDE)
                    g_vram[di & (CGA_SIZE - 1)] = al;
            }
        }
    ...
    for (uint8_t rows = 1; rows != 106; rows++) {
        uint16_t n = CURTAIN_ROW * rows;
 ...
 */
static void logo_pass(const uint8_t *src, uint16_t dest, int32_t rows,
                      int32_t erase, int32_t back)
{
    ...
    for (uint16_t n = rows; n > 0; n--) {
        ...
        uint16_t w = back ? di - 4 : di;
        for (uint16_t i = 0; i < 10; i++) {
            uint16_t a = w & (CGA_SIZE - 1), b = (w + 1) & (CGA_SIZE - 1);
            if (erase) {
                g_vram[a] ^= 0xff;
                g_vram[b] ^= 0xff;
            } else {
...
void intro_logo(void)
{
    ...
    uint8_t *top = &assets.logo[sizeof assets.logo - 2];      /* the last word */
    /* Backwards from the bottom right corner, then forwards from the middle
     * of the screen - the two halves the picture is built in. */
    logo_pass(top, cga_at(316, 199), 91, 0, 1);
    logo_pass(top, cga_at(316, 199), 90, 1, 1);
    logo_pass(assets.logo, cga_at(228, 109), 91, 0, 0);
    logo_pass(assets.logo, cga_at(228, 108), 92, 1, 0);
}

/* 1ac2:55e5  intro_reveal
 *
 * Two halves. The first paints a dithered wipe down 52 columns in four
 * phases; the second reveals the picture band by band, widening the slice
 * copied from one byte to 52 and waiting for retrace on each step.
 */
/* One band down. The step stays inside one half of the interlace, so eleven
 * rows of CGA_STRIDE is 22 scan lines - which is what separates the seven
 * bands the picture is revealed in, each of them 21 lines tall. */

void intro_reveal(void)
{
    uint16_t bx0 = cga_at(0, 14);       /* the wipe's top left */
    for (uint16_t dl = 52; dl > 0; dl--, bx0++) {
        ...
        for (uint16_t dh = 4; dh > 0; dh--) {
            al &= 0x55;
            uint16_t di = di0;
            for (uint16_t cl = 7; cl > 0; cl--, di += REVEAL_BAND)
                g_vram[di & (CGA_SIZE - 1)] = al;
    ...
    uint16_t bp = cga_at(0, 4);         /* the first band's top left */
    for (uint16_t band = 0; band < 7; band++, bp += REVEAL_BAND) {
        for (uint16_t bx = 1; bx < 53; bx++) {
            /* The slice is anchored on the band row's last byte - 51 of the
             * 52 - and widens to the left, a byte a pass, which is why the
             * original holds that far edge rather than the band's start. */
            const uint8_t *si = &assets.reveal[band][51 - (bx - 1)];
            uint16_t di = bp;
            io_wait_retrace();
            for (uint16_t dl = 21; dl > 0; dl--) {
                for (uint16_t i = 0; i < bx; i++)
 ...
 *
 * The character at the bottom right, scrolled up into place: 26 frames, each
 * moving a 49-byte by 25-row block up one scan line and feeding a fresh row
 * in at the bottom from 0xc46:0x488a.
 */
void intro_scroll(void)
{
    /* `mov bl, 0x1a` - one pass per row of scroll_rows, which is declared
     * with exactly 26 of them and pinned there by reveal starting 1274 bytes
     * later. Not the block's height plus one: those two numbers agree by
     * accident, and only one of them is the reason. */
    const uint8_t *feed = assets.scroll_rows[0];
    for (uint16_t bl = sizeof assets.scroll_rows / sizeof *assets.scroll_rows;
         bl > 0; bl--) {
        uint16_t di = cga_at(SCROLL_X, SCROLL_TOP);
        io_wait_retrace();
        for (uint16_t bh = SCROLL_ROWS; bh > 0; bh--) {
            uint16_t src = cga_next_row(di);
            for (uint16_t i = 0; i < SCROLL_BYTES; i++)
                g_vram[(di + i) & (CGA_SIZE - 1)] =
                    g_vram[(src + i) & (CGA_SIZE - 1)];
            di = src;
        }
        /* The fresh row goes on the line the block just vacated at the
         * bottom, which is why this is the top plus its own height. */
        uint16_t last = cga_at(SCROLL_X, SCROLL_TOP + SCROLL_ROWS);
        for (uint16_t i = 0; i < SCROLL_BYTES; i++)
            g_vram[(last + i) & (CGA_SIZE - 1)] = feed[i];
        feed += SCROLL_BYTES;
        /* `mov cx, 0x19` - a delay count, 25 by coincidence too. */
        for (uint16_t i = 0; i < 25; i++)
            game_delay();
    }
}
...
static void menu_redraw(void)
{
    ...
    restore_screen();
    if (global.input_selected_fn != INPUT_KEYBOARD_FN)
        menu_arrow();
    global.particle_count = 80;
    menu_particles_init(0xb800);
    global.banner_state = 2;
    global.banner_ptr = global_off(global.banner_text);
}
...
void game_main(const char *dir, const char *levels)
{
     ...
     * setting's 0 means, and it leaves runtime.delay_count at the readme's 110.
     *
     ...
     * delay to a `ret`, and play_loop keys off that byte to open the ball's
     ...
     * global.level_file, skip a leading dot, and append ".PPC" unless it already has
     * an
    ...
    if (levels && *levels) {
        char *name = global.level_file;
        size_t n = 0;
        while (levels[n] && n < sizeof global.level_file - 5) {
            name[n] = levels[n];
            n++;
        }
        name[n] = 0;
         ...
         * 1ac2:08c8 opens global.level_file itself. So `.LTF` becomes `.LTF.PPC` rather
    ...
    global.input_selected_fn = INPUT_KEYBOARD_FN;
    global.cheat_done = 0;
    runtime.sound_on = 1;
    ...
    for (;;) {
        ...
        while (!back_to_menu) {
            ...
            if (!io_key_ready()) {
                ...
                if (*global_ptr(global.banner_ptr) == 0) {
                    start_demo();
                    back_to_menu = 1;
                    continue;
                }
            ...
            if (global.cheat_done != 1)                     /* 1ac2:0239 */
                cheat_match(key & 0xff);   /* AL of AX: the character */
            ...
            case KEY_F10:
                ...
                break;
            case KEY_F3:                                /* mouse */
                if (global.input_selected_fn != INPUT_MOUSE_FN) {
                    global.input_selected_fn = INPUT_MOUSE_FN;
                    menu_arrow();
                }
                break;
            ...
            case KEY_F2:                                /* demo */
                ...
                break;
            case KEY_F1:                                /* play */
                global.input_active_fn = global.input_selected_fn;
                speaker_on();
                ...
                play_prepare();
                if (global.input_active_fn == INPUT_KEYBOARD_FN)
...
static void input_keys_mouse(void)
{
    ...
    if (key == ((KEY_ESC << 8) | KEY_ESCAPE_CHAR)) {   /* 1ac2:1669 */
        ...
        /* AL of what INT 16h returns. The original's `and al,0xdf` clears
         * bit 5, which is the case bit, so either case of A answers - and
         * those two bytes are the only ones it admits. */
        uint8_t k = io_get_key();       /* 1ac2:1673 */
        if ((k & ~0x20) == 'A') {       /* 1ac2:1675, and the cmp at 1677 */
...
static void input_keys_keyboard(void)
{
    ...
    uint8_t k = io_get_key();           /* 1ac2:16e1 */
    if ((k & ~0x20) == 'A') {           /* 1ac2:16e3, and the cmp at 16e5 */
        entities_clear();
        longjmp(g_back_to_menu, 1);
    }
    screen_unstash();                   /* 1ac2:4c13 */
    install_int09();                    /* 1ac2:03b0 */
    global.last_make = 0xff;          /* 1ac2:16ef */
}

void game_input(void)
{
    uint16_t which = global.input_active_fn;
...
int32_t play_loop(void)
{
    ...
    /* A dithered bar, the twelve characters of level_text, and another bar:
     * the font is twelve scan lines tall, which is why the second bar is at
     * the text's own row plus twelve. */
    uint16_t di = cga_at(BANNER_X, BANNER_Y - 1);
    for (uint16_t i = 0; i < 12; i++, di += 2) {
        g_vram[di & (CGA_SIZE - 1)] = 0xaa;
        g_vram[(di + 1) & (CGA_SIZE - 1)] = 0xaa;
    }
    draw_text(global.level_text, 12, cga_at(BANNER_X, BANNER_Y));
    di = cga_at(BANNER_X, BANNER_Y + FONT_H);
    for (uint16_t i = 0; i < 12; i++, di += 2) {
        g_vram[di & (CGA_SIZE - 1)] = 0xaa;
        g_vram[(di + 1) & (CGA_SIZE - 1)] = 0xaa;
    }
    ...
    /* And wipe it off again, starting on the upper bar: the two bars and the
     * twelve rows of text between them, which is where the fourteen comes
     * from. The `sub di, 24` in the original only puts back what `rep stosw`
     * advanced; taking it literally walks the wipe left a band a row and
     * leaves the banner on screen under everything else. */
    di = cga_at(BANNER_X, BANNER_Y - 1);
    for (uint16_t dl = 1 + FONT_H + 1; dl > 0; dl--) {
        for (uint16_t i = 0; i < 24; i++)
            g_vram[(di + i) & (CGA_SIZE - 1)] = 0;
        di = cga_next_row(di);
    }

    /* Half the field. The original writes a bare `mov cx, 0x64` at
     * 1ac2:18ef and stores it as both, so the halving is this port's
     * reading of what the hundred is for, not something the original
     * spells out. */
    global.paddle_x = global.paddle_prev_x = WALL_RIGHT / 2;
    ...
    b->state = 1;
    b->bounces = 0;
    global.balls[1].state = 0;
    global.balls[2].state = 0;
    ...
    if (global.input_active_fn != INPUT_DEMO_FN) {
        ...
        for (;;) {
            ...
            if (global.key_action == 1)
                break;
            draw_paddle(global.paddle_sprites[0][0]);
            io_present();
            if (!io_pump())
                return 1;
        }
...
frames:
    for (;;) {                          /* one iteration is one frame */
        global.frame_delay = global.frame_delay_set;
        anim_step();
        ...
        if (--global.speed_step != 0) {
            for (uint16_t i = 0; i < 3; i++) {
                ...
                if (st == 0 || st >= 3)
                    continue;
                if (global.caught == 1 && !ball_on_paddle(ball))
                    continue;
                ball_step(ball);
                if (!ball_redraw(ball)) {
         ...
         * global.entity_remove - twelve of them do - and the walk does the
        ...
        /* Where the original spent global.frame_delay empty loops and then POPSPEED's
...
void play_session(void)
{
    ...
    for (;;) {
         ...
         * always assets.levels[level_number], which is this
         * record. The game keeps both because the results screen walks the
         * offset. */
        global.level = assets.levels[global.level_number];
 ...
 *
 * Cell 0x0c is not a brick - it hands off to 0x41e5. Cells from 24 up have
 * their bitmaps in the block the program reaches as segment 0x14a1 rather than
 * in cell_bitmap.
 */
uint16_t draw_brick_row(uint8_t y)
{
    uint16_t di = cga_at(WALL_LEFT, y);
...
void level_intro(void)
{
    ...
    global.sweep_y[3] = 0xc2;
    global.sweep_y[2] = 0xbd;
    global.sweep_y[1] = 0xb8;
    ...
    while (global.sweep_y[0] != 12) {
        ...
        field_backdrop(global.sweep_y[0] - 7);
        for (uint16_t k = 0; k < 4; k++) {
 ...
 * packed record at an odd address - global.balls[i] is at 0x2ea1 + i * 0x1e - so a
 ...
 *
 * The playfield is x in (8, 196) and y from 4 down to 195; y == 196 is the
 ...
 * does and the reason `+24`/`+0x19` is where the last bounce was rather than
 * where the ball is.
 *
 * `+0x1d` counts bounces, and every 35 of them the slope is picked afresh
...
void ball_after(ball_t *b)
{
    ...
    if (global.net_on == 1) {     /* the net catches it */
        runtime.sound_request = SOUND_BOUNCE;
        b->anchor_x = b->x;
        b->anchor_y = 195;
        b->dir_y = 1;                 /* upwards */
 ...
 *
 * Each entry is a word stored (dy, dx), matching the ball's own `+22`,
...
void probe_cell_at(uint8_t x, uint8_t y, hit_t *hit)
{
    ...
    global.hit_count++;
    hit->centre.x = ((x & 0xf0) + 8);
    hit->centre.y = ((y & 0xf8) + 6);
}
...
void drop_duplicate_hits(void)
{
    for (uint16_t i = 0; i < 3; i++) {
        if (!global.hits[i].cell_ptr)
            continue;
        point_t centre = global.hits[i].centre;
        /* 1ac2:27b7 compares the pair as one word, which is what makes "the
         * same brick" a single compare; here it is the two bytes it holds. */
        for (uint16_t j = i + 1; j < 4; j++)
            if (global.hits[j].cell_ptr &&
                global.hits[j].centre.x == centre.x &&
                global.hits[j].centre.y == centre.y)
...
void ball_bricks(ball_t *b)
{
    ...
    /* 1ac2:267d compares the whole word against zero, so both halves of
     * the first pair - and it is (0, 0), so this arm never fires. Kept
     * because the original tests it. */
    if (n == 3 || (n == 2 && ((s0 && s2) ||
                              (!s0 && s1 && (global.hit_dirs[0].x ||
                                             global.hit_dirs[0].y))))) {
...
static void brick_common(ball_t *ball, uint8_t sound,
                         uint16_t a, uint16_t b, uint16_t c)
{
    brick_score(a, b, c);
    runtime.sound_request = sound;
    if (ball)
        ball->bounces = 0;
}
...
static entity_t *brick_entity(hit_t *hit, uint16_t handler_fn,
                             uint16_t frames_ptr, uint8_t rate)
{
...
static void brick_1_or_2(hit_t *hit, ball_t *ball, int32_t is_two)
{
    ...
    *cell = 0;
    ...
    xor_sprite_16xn(centre.x, centre.y + 1, appear, 6);
    global.bonus_cap++;
}
...
void score_add(void)
{
    ...
    for (int16_t i = 5; i >= 0; i--) {          /* least significant first */
        uint8_t sum = (global.score_text[i] & 0x0f) + global.score_add[i]
                     + (carry ? 1 : 0);
        uint8_t adjusted = sum + 6;
        carry = (adjusted & 0xf0) || (sum & 0xf0);
        global.score_text[i] = ('0' | ((carry ? adjusted : sum) & 0x0f));
    }
    /* Redraw the six digits into the panel. */
    uint16_t di = cga_at(264, 138);
    ...
    uint16_t thresh = global.extra_at;
 ...
 * are four to a row: `al & 0xfc` steps along and `(al & 3)` steps down.
 */
void extra_life(void)
{
    ...
    uint16_t di = cga_at(240, 169) + (n & 0xfc) + (n & 3) * LIVES_ROW;
...
void level_draw(void)
{
    ...
    /* Rub out one life marker: the same layout extra_life draws them in,
     * and the same trap. The
...
void panel_draw(void)
{
    ...
    /* Bottom right, and it grows upwards. */
    uint16_t bottom = cga_at(208, 199);
    for (uint16_t rows = 1; rows != PANEL_ROWS; rows++) {
        uint16_t d = bottom;
        io_wait_retrace();
        for (uint16_t r = 0; r < rows; r++) {
...
{
    const level_anim_t *a = &animations.level[global.level_number];
    global.anim_ptr = a->script_ptr;
    global.anim_rate = a->rate;
    global.anim_count = a->rate;
}
 ...
 * right, then the one that launches the ball off the paddle. It copies a
 * zero-terminated string into the **text-mode** framebuffer - `stosb` then
 * `inc di` steps two bytes, so the attribute byte between characters is left
 * alone - and it is the only routine that does, because that screen is the
 * only one drawn in mode 01h rather than in graphics.
 *
 * It was called copy_string_text here, which described the instructions and
 * not the job. The screen it belongs to is **deliberately not transcribed**;
 * see screen_define_keys. This is kept because it is complete and it checks
 * out.
 */
void define_keys_prompt(uint16_t src, uint16_t dst)
{
    (void)src;
    (void)dst;
}
...
/* 1ac2:3146  flash_bar
 *
 * XOR a pattern across 24 words along the bottom of the playfield. Called
 * with the pattern in DX, so the same routine both draws it and rubs it out.
 */
void flash_bar(uint16_t pattern)
{
    uint16_t di = cga_at(WALL_LEFT, CGA_H - 1);
    for (uint16_t i = 0; i < 24; i++, di += 2) {
        g_vram[di & (CGA_SIZE - 1)] ^= pattern;
        g_vram[(di + 1) & (CGA_SIZE - 1)] ^= (pattern >> 8);
    }
}
...
entity_t *entity_alloc(void)
{
    ...
    uint16_t taken_ptr = global.entity_free_ptr;
    global.entity_free_ptr = entity_ptr(taken_ptr)->next_ptr;
...
{
    entity_ptr(global.entity_prev_ptr)->next_ptr = node->next_ptr;
    node->next_ptr = global.entity_free_ptr;
    global.entity_free_ptr = global_off(node);
    global.entity_remove = 0;
}
...
void sprite_shift_draw(uint8_t x, uint8_t y, const uint8_t *src)
{
    ...
    xor_sprite_20x16(x, y, global.sprite_work[0]);
}
                           ...
                           void (*draw)(uint8_t, uint8_t, const uint8_t *))
{
    ...
    uint16_t x = a->sprite.x, y = a->sprite.y;
 ...
 * ball is in play, waits out [bx+6], and then steps its animation every 35
...
void entity_hatch(ent_hatch_t *h)
{
    ...
    /* 1ac2:3934 is `sub al, 0xa`, so the ten comes off in a byte register.
     * The eight marks sit at y = 30, 78, 130 and 179, so it never wraps -
     * this is AL's width written down, not a wrap being relied on. */
    uint8_t row = h->y - 10;
    uint16_t di = cga_at(h->x, row);
 ...
 */
static const uint8_t *cell_at(uint8_t y, uint8_t x)
{
    uint8_t row = y & 0xf8;
    return &global.level.cells[row + (row >> 1) + (x >> 4)];
}

/* 1ac2:3c66  bonus_move_right */
int32_t bonus_move_right(ent_anim_t *b, uint8_t *px, uint8_t *py)
{
    (void)b;
    uint8_t y = *py, x = *px;
    if (x >= WALL_RIGHT - CAPSULE_SIZE)
        return 0;
    const uint8_t *cell = cell_at(y - 6, x + 8);
    if (cell[0] || cell[12])
        return 0;
    if ((((y - 6) & 7) != 0) && cell[24])
        return 0;
...
int32_t bonus_move_left(ent_anim_t *b, uint8_t *px, uint8_t *py)
{
    (void)b;
    uint8_t y = *py, x = *px;
    if (x <= WALL_LEFT)
        return 0;
    const uint8_t *cell = cell_at(y - 6, x - 9);
    if (cell[0] || cell[12])
        return 0;
    if ((((y - 6) & 7) != 0) && cell[24])
        return 0;
...
int32_t bonus_move_up(ent_anim_t *b, uint8_t *px, uint8_t *py)
{
    (void)b;
    uint8_t y = *py, x = *px;
    if (y <= 6)
        return 0;
    const uint8_t *cell = cell_at(y - 7, x - 8);
    if (cell[0])
        return 0;
...
int32_t bonus_move_down(ent_anim_t *b, uint8_t *px, uint8_t *py)
{
    uint8_t y = *py, x = *px;
    ...
    const uint8_t *cell = cell_at(y + 10, x - 8);
    if (cell[0])
        return 0;
    if ((((x - 8) & 0x0f) != 0) && cell[1])
        return 0;
...
int32_t bonus_steer(ent_anim_t *b, uint8_t *px, uint8_t *py)
{
...
void xor_sprite_16xn(uint8_t x, uint8_t y, const uint8_t *src, uint8_t rows)
{
    uint16_t di = cga_at(x, y);
    for (uint16_t r = 0; r < rows; r++) {
        for (uint16_t b = 0; b < 4; b++)
...
{
    ball_t *b = ball;
    b->x = b->prev_x = b->anchor_x = x;
    b->y = b->prev_y = b->anchor_y = y;
    b->acc_x = b->acc_y = 0;
    b->state = 1;
...
void entity_ball_arrive(ent_anim_t *a)
{
    ...
    ball_place(ball_ptr(a->arg.ball_ptr), a->sprite.x + 8, a->sprite.y - 4);
}
...
void brick_9(hit_t *hit, ball_t *ball)
{
    ...
    brick_entity(hit, ENTITY_PLAIN_FN,
                 global_off(&global.teleport_out_ptr[1]), 50)->p.anim.arg.cell_ptr =
        hit->cell_ptr;
...
{
    uint8_t y = a->sprite.y, x = a->sprite.x;
    ...
    if (global.hit_kind == 0)
        return;
    if (global.hit_kind == 2)
    ...
    ball_place(ball_ptr(a->arg.ball_ptr), a->sprite.x + 8, ry + 12);
...
uint16_t pixel_xor(uint8_t x, uint8_t y)
{
    ...
    g_vram[di & (CGA_SIZE - 1)] ^= mask;
    return di;
}

/* 1ac2:306b  shot_xor
 *
 * The laser: two dots three scan lines tall, 19 pixels apart - one under
...
void shot_xor(uint8_t x, uint8_t y)
{
    for (uint16_t side = 0; side < 2; side++) {
        uint8_t sx = side ? x + LASER_SPAN : x;
        uint8_t mask = 0xc0 >> ((sx & 3) * 2);
...
{
    /* 1ac2:3f20 does all four compares in AH against AL, and every sum that
     * feeds one is `add al, 3` or `add ah, 0xf` - byte registers, so each
     * far edge wraps at 256. That is what these types are for. */
    uint8_t cap_x = s->x, cap_y = s->y;
    uint8_t cap_right  = cap_x + (CAPSULE_SIZE - 1);
    uint8_t cap_bottom = cap_y + (CAPSULE_SIZE - 1);
    uint8_t ball_right  = ball->x + (BALL_WIDTH - 1);
    uint8_t ball_bottom = ball->y + (BALL_HEIGHT - 1);

    if (cap_y > ball_bottom || cap_bottom < ball->y)
        return;
    if (cap_right < ball->x || cap_x > ball_right)
        return;
    global.hit_kind = 2;
}
...
void bonus_update(ent_sprite_t *s, uint8_t nx, uint8_t ny)
{
    global.hit_kind = 0;
    ...
    if (global.laser_on == 2) {
        uint8_t shot_y = global.laser_y + 2, shot_bottom = shot_y + 1;
        uint8_t cap_y = s->y, cap_bottom = cap_y + (CAPSULE_SIZE - 1);
        if (shot_bottom >= cap_y && shot_y <= cap_bottom) {
            uint8_t shot_x = global.laser_x, cap_x = s->x;
            uint8_t cap_right = cap_x + (CAPSULE_SIZE - 1);
            uint8_t shot_x2 = shot_x + LASER_SPAN;   /* the other dot */
            int32_t hit = (shot_x  >= cap_x && shot_x  <= cap_right) ||
                          (shot_x2 >= cap_x && shot_x2 <= cap_right);
            if (hit) {
                global.hit_kind = 3;
    ...
    /* The paddle. */
    uint8_t cap_y = s->y, cap_bottom = cap_y + (CAPSULE_SIZE - 1);
    if (cap_y <= PADDLE_BOTTOM && cap_bottom >= PADDLE_Y) {
        uint8_t cap_x = s->x, cap_right = cap_x + (CAPSULE_SIZE - 1);
        uint8_t paddle_right = global.paddle_x + global.paddle_width;
        if (cap_right >= global.paddle_x && cap_x <= paddle_right) {
            global.hit_kind = 1;
            sprite_shift_draw(cap_x, cap_y, global_ptr(global_w(s->frame_ptr)));
            return;
        }
    }
...
settle:
    ...
    runtime.sound_request = 6;         /* 1ac2:3a67 */
 ...
 */
/* The band both routines move: 27 rows from 172, so its last row is 198. One
 * starts at the top and walks down, the other at the bottom and walks up. */

void scroll_up_band(void)
{
    uint16_t di = cga_at(WALL_LEFT, BAND_TOP);
    for (uint16_t r = BAND_ROWS; r > 0; r--) {
...
{
    uint16_t si = cga_at(WALL_LEFT, BAND_TOP + BAND_ROWS - 1);
    for (uint16_t r = BAND_ROWS; r > 0; r--) {
...
{
    uint16_t di = cga_at(global.paddle_x, PADDLE_Y);
    for (uint16_t r = 0; r < 16; r++) {
        for (uint16_t b = 0; b < 7; b++)
...
void draw_paddle_shifted(const uint8_t *sprite)
{
    ...
    blit_xor(global.paddle_pix[1], &global.paddle_rows[1]);
...
void entities_clear(void)
{
    ...
    while (node_ptr != END_PTR) {
        ...
        e->next_ptr = global.entity_free_ptr;
...
void life_lost(void)
{
    ...
    while (node_ptr != END_PTR) {
        ...
        e->next_ptr = global.entity_free_ptr;
        global.entity_free_ptr = node_ptr;
        entity_ptr(global.entity_prev_ptr)->next_ptr = next_ptr;
        node_ptr = next_ptr;
    }
    level_between();
}
 ...
 */
void read_new_key(uint16_t which)
{
...
void entity_capsule_frames(ent_fall_t *f, uint16_t table_ptr)
{
    ...
    uint8_t rows = list[f->frame].rows;

    uint16_t y = f->y;
    f->y++;
    xor_sprite_16xn(f->x, y, global_ptr(src_ptr), rows);
...
void fill_column(uint16_t di, uint16_t value)
{
    for (uint16_t i = 0; i < 25; i++) {
        g_vram[di & (CGA_SIZE - 1)] = value;
        g_vram[(di + 1) & (CGA_SIZE - 1)] = (value >> 8);
        di = cga_next_row(di);
    }
}
...
void bonus_points(void)
{
    ...
    if (global.net_on == 1) {
        flash_bar(FLASH_DASHES);
        global.net_on = 0;
        fill_column(cga_at(PANEL_NET_X, PANEL_BAR_Y), 0);
    }
    if (global.extra_on != 1)
        return;
    global.extra_on = 0;
    fill_column(cga_at(PANEL_M_X, PANEL_BAR_Y), 0);
}
...
void bonus_net(void)
{
    ...
    fill_column(cga_at(PANEL_NET_X, PANEL_BAR_Y), 0xaaaa);
    global.net_pos = cga_at(PANEL_NET_X, PANEL_BAR_Y);
}
...
void entity_multiball(void)
{
    ...
    global.ball_alive = 3;
    uint8_t dy = src->dy, dx = src->dx;
    uint16_t x = src->x, y = src->y;
 ...
 * stepping only when the count is a multiple of 35 (a `div cl` and a test of
 ...
 * cell of 24 or more takes its bitmap from the block reached as segment
...
void level_between(void)
{
    ...
    /* Two scan lines of nothing along the top of the field. */
    uint16_t first = cga_at(WALL_LEFT, WALL_TOP);
    uint16_t second = cga_at(WALL_LEFT, WALL_TOP + 1);
    for (uint16_t i = 0; i < 24; i++) {
        g_vram[(first + i * 2) & (CGA_SIZE - 1)] = 0;
        g_vram[(first + 1 + i * 2) & (CGA_SIZE - 1)] = 0;
        g_vram[(second + i * 2) & (CGA_SIZE - 1)] = 0;
        g_vram[(second + 1 + i * 2) & (CGA_SIZE - 1)] = 0;
    }

    uint16_t y = BRICK_TOP;
    for (uint16_t row = 0; row < BRICK_ROWS; row++, y += BRICK_HEIGHT) {
        uint16_t x = WALL_LEFT;
        for (uint16_t col = 0; col < BRICK_COLS; col++, x += 16) {
            uint8_t cell = global.level.cells[row * BRICK_COLS + col];
            if (cell == 0x0c) {
                cell_hole_draw(x, y);
                continue;
            }
    ...
    /* And the empty band under the bricks: it starts where they end, which
     * is BRICK_ROWS of BRICK_HEIGHT below the field's first scan line. 41
     * rows of 48 bytes, both halves of the interlace each pass. */
    uint16_t bp = cga_at(WALL_LEFT, BRICK_TOP + BRICK_ROWS * BRICK_HEIGHT);
...
int32_t name_field(uint16_t di, uint8_t *abort)
{
    uint8_t *name = global.players[global.player_digit - '1'].name;
    uint8_t len = 0;
    *abort = 0;
    ...
    for (;;) {
        ...
        /* 1ac2:13e2 is `cmp al, 0x60` with a `jb` that skips only below it,
         * so 0x60 - the backtick - folds too. That is not an off-by-one
         * against 'a': clearing bit 5 maps the block 0x60..0x7f onto
         * 0x40..0x5f whole, backtick onto '@' as much as 'a' onto 'A', and
         * the backtick is where that block begins. So the test asks "is this
         * in the half the case bit moves things to", not "is this a
         * lowercase letter" - and what is not a letter afterwards is rejected
         * by the range checks below. */
        if (c >= '`')
            c &= ~0x20;                 /* clear the case bit */
        int32_t ok = (c == ' ' || c == '-' ||
                  (c >= '0' && c <= '9') || (c >= 'A' && c <= 'Z'));
        if (!ok || len == 12)
            continue;
     ...
     *
     * Thirteen, against a name of twelve: `mov dl,0xd` at 1ac2:142b and a
     * do-while, so the original writes one space **past** the name and onto
     * the byte after it, which is `lives`. Deliberate here rather than a
     * miscount - player_record_init sets lives afterwards, so nothing
     * survives it - and the count is the original's. */
    uint16_t pad = 13 - len;
    uint8_t shift = (pad - 1) >> 1;
    for (uint16_t i = 0; i < pad; i++, name++, di += 2) {
        *name = ' ';
        draw_char(' ', di);
    }
...
static uint16_t panel_row(uint16_t di, uint8_t lead, uint8_t mid,
                          uint8_t tail, int32_t has_tail)
{
    uint16_t d = di;
    g_vram[d++ & (CGA_SIZE - 1)] = lead;
    for (uint16_t i = 0; i < (has_tail ? 0x2e : 0x2f); i++)
        g_vram[(d + i) & (CGA_SIZE - 1)] = mid;
    if (has_tail)
        g_vram[(d + 0x2e) & (CGA_SIZE - 1)] = tail;
    return cga_next_row(di);
}
 ...
 * player types into is the label line again, 22 bytes in.
 *
...
uint8_t screen_player_names(void)
{
    ...
    uint16_t di = cga_at(8, 8);
    for (;;) {
        uint16_t top = di;                      /* pushed at 1ac2:10f2 */
        ...
        if (++global.player_count == 9) {
            global.player_digit = '1';
            return 0;
        }

        /* The box just filled in becomes an engraved panel. The step stays
         * inside one half of the interlace, so eight rows of CGA_STRIDE is
         * sixteen scan lines down from the box's top. */
        uint16_t d = top + 8 * CGA_STRIDE;
...
uint16_t frame_band(uint16_t di, uint16_t fill)
{
    ...
    for (uint16_t i = 0; i < 23; i++, di += 2) {
        g_vram[di & (CGA_SIZE - 1)] = fill;
        g_vram[(di + 1) & (CGA_SIZE - 1)] = (fill >> 8);
    }
    for (uint16_t i = 0; i < 3; i++)
 ...
 * by scrolling a 26-word column up 0xc2 times and laying a fresh two-byte
...
{
    runtime.frame_phase = 0;

    uint16_t di = cga_at(0, 194);
    static const uint16_t fills[6] = { 0xffff, 0x5555, 0x5454, 0x5555, 0, 0 };
    for (uint16_t i = 0; i < 6; i++) {
    ...
    for (uint16_t pass = 194; pass > 0; pass--) {
        ...
        uint8_t cap = (runtime.frame_phase & 3) ? 0x50 : 0x10;
        g_vram[di++ & (CGA_SIZE - 1)] = 0x0d;
        g_vram[di++ & (CGA_SIZE - 1)] = cap;
        for (uint16_t i = 0; i < 24; i++, di += 2) {
            g_vram[di & (CGA_SIZE - 1)] = 0;
            g_vram[(di + 1) & (CGA_SIZE - 1)] = 0;
        }
...
void brick_11(hit_t *hit, ball_t *ball)
{
     ...
     * counter here made the every-35-bounces slope shuffle fire at a
...
{
    mark_t *m = &global.field_marks[game_random(io_ticks(), 4)];
    if (m->taken != 0)
...
uint16_t particle_random(uint16_t ax, uint16_t ticks, uint16_t limit)
{
    ...
    if (!limit)
        return 0;
    global.particle_seed += ax / limit;
    return ax % limit;
}

/* 1ac2:548a  particle_init
 *
 * Set one kernel going from (104, 160) with a random speed and a random
 ...
 */
uint16_t particle_init(particle_t *p, uint16_t ax_in)
{
    p->x0 = 104;
    p->y0 = 160;
    int16_t ax = particle_random(ax_in, io_ticks(), 6) + 8;
    p->speed = ax;
    ax = particle_random(ax, io_ticks(), 70) - 35;
    if (ax == 0)
        ax = 10;
    p->t0 = ax;
    p->t = ax;
...
void menu_particles_tick(void)
{
    ...
    for (uint16_t k = 0; k < n; k++) {
        ...
        uint16_t x = particle_x(p), y = particle_y(p);
        if (x <= 319 && y <= 199)
...
void plot_pixel_xor(uint16_t x, uint16_t y, uint8_t colour)
{
    ...
    uint16_t di = cga_at(x, y);
    uint8_t shift = 6 - (x & 3) * 2;
    g_vram[di & (CGA_SIZE - 1)] ^= ((colour & 3) << shift);
}
...
{
    /* The **highest** byte of the run, because the shift walks down from it. */
...
{
    const uint8_t *src = &assets.hole_picture[row][col * 4];
    for (uint16_t b = 0; b < 4; b++)
        g_vram[(di + b) & (CGA_SIZE - 1)] = src[b];
}
...
{
    global.cga_colour += 0x10;
    if (global.cga_colour == 0) {
        global.cga_mode ^= 4;
        io_cga_mode(global.cga_mode);
    }
    io_cga_colour(global.cga_colour);
}
...
hsc_entry_t *hsc_bubble(const hsc_entry_t *a, hsc_entry_t *b)
{
     ...
     * b[-1]. And the `sub di, 24` that follows a `rep movsw` which itself
 ...
 * global.hsc. The drive check at 0x4e04 comes first, and a failure is silent. */
 ...
 * going up, column 50 going down, row 0 going right and row 96 going left.
 *
 * The vertical step is four rows of CGA_STRIDE. Row 96 is offset 7680, under
 * CGA_PLANE, so every offset here is in the even half - which makes those
 * four rows eight scan lines on the screen.
 */
uint16_t border_step(uint16_t di)
{
    uint8_t row = di / CGA_STRIDE, col = di % CGA_STRIDE;
    if (row == 0)
        return (col == 50) ? di + BORDER_STEP_Y : di + 2;
    if (row == 96)
        return (col == 0) ? di - BORDER_STEP_Y : di - 2;
    return (col == 50) ? di + BORDER_STEP_Y : di - BORDER_STEP_Y;
}
...
void border_animate(void)
{
    for (uint16_t i = 0; i < 14; i++) {
        ...
        di = border_step(di);
        border_erase(di);
        runtime.border_pos[i] = di;
    }
}

/* 1ac2:5019  border_block, and 1ac2:5045  border_row
 *
 * The top and bottom edges: 26 columns of the eight-word sprite side by
 * side, and 23 rows of it stacked. Between them they lay the frame the
 * markers then run around.
 */
void border_row(uint16_t di)
{
    /* 23 passes of eight words each, the pattern read from border_spr - the
     * code segment, the bytes immediately after this routine, which is what
     * cs:0x506d names - and DI carried on down the interlace throughout
     * rather than reset per pass. What was here before called border_draw 26
     * times, which is neither the count nor the operation. */
    for (uint16_t n = 23; n > 0; n--) {
        for (uint16_t r = 0; r < 8; r++) {
            uint16_t w = runtime.border_spr[r];
            g_vram[di & (CGA_SIZE - 1)] = w;
            g_vram[(di + 1) & (CGA_SIZE - 1)] = (w >> 8);
            di = cga_next_row(di);
        }
    }
}

void border_block(uint16_t di)
{
    /* 26 columns **side by side**: 1ac2:501e pushes DI and 1ac2:503d pops
...
void level_tally(void)
{
    for (uint16_t i = 0; i < 168; i++) {
        uint8_t v = global.level.cells[i];
 ...
 * time: twenty rows from 160 stepping CGA_STRIDE stay in the even half, then
 * twenty from 161 do the odd. That is why the step is a stride and not a
 * cga_next_row.
 */
void screen_stash(void)
{
    ...
    uint16_t si = cga_at(0, STASH_TOP);
    for (uint16_t half = 0; half < 2; half++) {
        for (uint16_t r = 0; r < STASH_ROWS; r++) {
            for (uint16_t b = 0; b < STASH_BYTES; b++)
                keep[b] = g_vram[(si + b) & (CGA_SIZE - 1)];
            keep += STASH_BYTES;
            si += CGA_STRIDE;
        }
        si = cga_at(0, STASH_TOP + 1);
    }
    uint16_t di = cga_at(0, STASH_TOP);
...
{
    uint8_t row = y - 6;
    const uint8_t *src = &assets.hole_picture[row][(x >> 2) - 2];
    uint16_t di = cga_at(x, y);
    for (uint16_t r = 0; r < 8; r++) {
...
void border_setup(void)
{
     ...
     * border_block at DI 0 and 0x1e00 and border_row at 0x140 and 0x172 -
     * which as positions are the top and bottom bands, and the left and
     * right uprights that run between them. The bottom band starting on 192
     * is why screen_high_scores bars its panel only down to there. */
    border_block(cga_at(0, 0));
    border_block(cga_at(0, 192));
    border_row(cga_at(0, 8));
    border_row(cga_at(200, 8));
 ...
 * the keyboard 1200 times, returning carry if a key came: on an 8086 those
 ...
 * of word pairs down a column and its twin 50 bytes to the right. `rows` is
 * how many of the middle segment to repeat, so the pillar grows a row per
 * pass and appears to extend downwards.
 * ===================================================================== */
static uint16_t pillar_pair(uint16_t di, uint16_t v)
{
    vram_setw(di, v);
    vram_setw(di + 50, v);
    return cga_next_row(di);
...
void field_marks_wide(uint16_t di, uint16_t rows)
{
 ...
 * (104, 152) instead of (104, 160) - eight scan lines higher - and with a
 * speed of `random(3) + 8` rather than `random(6) + 8`, so the kernels there
 * rise more slowly.
 */
uint16_t ending_particle_init(particle_t *p, uint16_t ax_in)
{
    p->x0 = 104;
    p->y0 = 152;
    int16_t ax = particle_random(ax_in, io_ticks(), 3) + 8;
    p->speed = ax;
    ax = particle_random(ax, io_ticks(), 70) - 35;
    if (ax == 0)
        ax = 10;
 ...
 *
 * Eight rows of one word from 0x28d9, XORed at a position the original packs
 * into AX - AL the x and AH the row - which is why the ending's script can be
 * a list of words and why ending_walk can step one half of it at a time. The
 * two halves are the two arguments here, and what they then compute is
 * `shr al,1` twice, the interlace bit and a multiply by 0x50: cga_at.
 */
void ending_blob(uint8_t x, uint8_t y)
{
    uint16_t di = cga_at(x, y);
...
point_t ending_blobs(void)
{
    ...
    /* The blobs' packed positions, a zero-terminated list of words. The
     * original's offsets here are the **0xc46 block's**, not the image's: DS
     * is that block for the whole ending sequence, which is why 0x289d is
     * blob_script rather than an address 49KB lower. */
    const point_t *si = assets.blob_script;
    point_t prev = { 0, 0 };
    for (;;) {
        for (uint16_t i = 0; i < 15; i++)
            game_delay();
        point_t pos = *si++;
        if (pos.x == 0 && pos.y == 0)          /* the terminating word */
            return prev;
        io_wait_retrace();
        ending_blob(pos.x, pos.y);
        if (prev.x || prev.y)
            ending_blob(prev.x, prev.y);
        prev = pos;
        io_present();
        if (!io_pump())
            return prev;
...
void ending_plot(uint16_t x, uint16_t y)
{
    ...
    for (uint16_t i = 0; i < 4; i++) {
        ...
        g_vram[d & (CGA_SIZE - 1)] ^= bits;
        g_vram[(d + 1) & (CGA_SIZE - 1)] ^= (bits >> 8);
 ...
 * ===================================================================== */
point_t ending_walk(uint8_t bl, uint8_t bh, point_t p)
{
    /* The position is a **parameter**, threaded in DX from ending_blobs by
     * the call site at 1ac2:59c9 - not a variable at 0x2821, which is where
     * this used to read it from. Modelling a register as memory works right
     * up until something else is in the register.
     *
     * Both steps are byte subtracts - `sub ah,4` at 1ac2:5bcc and `sub al,4`
     * at 1ac2:5c0c - so neither borrows into the other half. Written as one
     * word, `p - 4` does borrow: at an x under four it would walk the row
     * back as well, which the original cannot do. */
    uint8_t target = 80 - (bl << 3);
    assets.blob_target = target;
    while (p.y != target) {
        point_t next = { p.x, p.y - 4 };        /* `sub ah,4` */
        for (uint16_t i = 0; i < 15; i++)
            game_delay();
        io_wait_retrace();
        ending_blob(next.x, next.y);
        ending_blob(p.x, p.y);
        p = next;
    }
    ...
    assets.blob_target = target;
    while (p.x != target) {
        point_t next = { p.x - 4, p.y };        /* `sub al,4` */
        for (uint16_t i = 0; i < 15; i++)
            game_delay();
        io_wait_retrace();
        ending_blob(next.x, next.y);
...
void screen_all_levels_done(void)
{
     ...
     * - five steps, not 26 - and each step reads one byte with `lodsb` and
 ...
 */
void int09_handler(uint8_t scan)
{
    uint8_t make = !(scan & KEY_BREAK);

    if (scan == global.key_scan_l)
        global.last_dir = 0;
    if (scan == global.key_scan_r)
        global.last_dir = 1;
    if (scan == (KEY_F9 | KEY_BREAK))   /* released, not pressed */
        runtime.sound_on ^= 1;
    if (make)
        global.last_make = scan;

    /* The original walks the three scan codes and stores into the flags
     * *backwards* - `[global.key_action + (2 - i)]` - because the two triples are
     * laid out in opposite orders. Unrolled, so that is a fact you can see
     * rather than one you have to work out. */
    uint8_t code = scan & ~KEY_BREAK;
    if (code == global.key_scan_l) { global.key_left = make; return; }
    if (code == global.key_scan_r) { global.key_right = make; return; }
    if (code == global.key_scan_a) { global.key_action = make; return; }
}
 ...
 *
 * First it **emerges from the left wall**: eight passes at a fixed x 0,
 ...
 * reaches 0x4902.
 *
 * Then it **travels right**: `bh` from 0 to 21 puts the whole eight bytes
 * at byte `bh` of the row, and each row writes a **zero byte first** - the `stosb`
 ...
 * apiece, with a retrace wait and 25 delays between them.
 * ===================================================================== */
void intro_paddle(void)
{
    ...
    for (uint16_t bh = 0; bh < 22; bh++) {
        for (uint16_t phase = 0; phase < 4; phase++) {
            io_wait_retrace();
            uint16_t di = cga_at(0, INTRO_PADDLE_Y) + bh;
...
int32_t level_load_file(const char *dir)
{
    ...
    FILE *f = ppc_open(dir, global.level_file);
    if (!f) {
        fputs("****** Fichier des Tableaux non trouve ******\n", stderr);
        return 0;
    }
    fread(assets.ppc_signature, 1, 0x21b6, f);
    fclose(f);
    if (memcmp(assets.signature, assets.ppc_signature, 6) != 0) {
...
/* The panel's top left. Its own edge, which happens to be the playfield's
 * too - a different reason for the same number, so it is written separately.
 * The bar goes on HSC_TOP and the first line of text on the scan line under
 * it. */
...
void screen_high_scores(void)
{
    ...
    /* Its own top bar, with the text on the line under it - the same shape
     * as the results panel, two scan lines lower down the screen. */
    for (uint16_t i = 0; i < HSC_WIDTH; i++)
        vram_setw(cga_at(HSC_X, HSC_HIGH_TOP) + i * 2, 0xaaaa);

    /* HIGH SCORE, a glyph at a time - the original really does have ten
     * separate `mov al` / `call 0xc64` pairs for it. */
    uint16_t di = cga_at(HSC_X, HSC_HIGH_TOP + 1);
    di = draw_run(' ', 7, di);
    ...
    /* Bar the rest of the panel, stopping on y 192 - eight scan lines short
     * of the bottom, which the border below keeps for itself. */
    while (di != cga_at(HSC_X, 192))
        di = hsc_bar(di);

    /* The border animates until a key: 385 delays a step, 255 steps. */
...
void screen_end_of_game(void)
{
    ...
    for (uint16_t pass = 135; pass > 0; pass--) {
        ...
        for (uint16_t i = 0; i < EOG_BAND_LEN; i++, src++, dst++) {
            uint8_t old = *dst, add = *src, out = old;
    ...
    for (uint16_t g = 0; g < 7 && !keyed; g++) {
        ...
        const uint8_t *blank = global.eog_blank[0];
...
{
    /* Four scan lines of cap, two words wide, on 112 to 115. The original
     * steps them with `+0x2000`, `-0x1fb0`, `+0x2000` written out - which is
     * cga_next_row three times, because the first is in the even half. */
    static const uint16_t cap[4] = { 0xffff, 0x5555, 0x1515, 0x5555 };
    uint16_t di = cga_at(96, 112);
    for (uint16_t r = 0; r < 4; r++) {
        vram_setw(di, cap[r]);
        vram_setw(di + 2, cap[r]);
        di = cga_next_row(di);
    }

    uint16_t bp = cga_at(0, 111);
    for (uint8_t ah = 112; ah > 0; ah--) {
        ...
        d = wrote;
        uint8_t cap = (ah & 3) ? 0x50 : 0x10;
        g_vram[d & (CGA_SIZE - 1)] = 0x0d;          /* 1ac2:46b9 */
        g_vram[(d + 1) & (CGA_SIZE - 1)] = cap;
        ...
        if (cells)
            d = draw_brick_row(global.sweep_y[0]);   /* 1ac2:46d6 */
...
int32_t ball_after_endgame(ball_t *b)
{
    ...
    if (y == 116) {
         ...
         * so the bounce is for x **outside** 96..107. Inverted, the ball
    ...
    ball_draw(global.balls[0].sprite, global.balls[0].x, global.balls[0].y);
    if (global.cheat_done != 1)
        global.lives++;               /* a free life, unless cheating */
    global.sweep_y[0] = 0x75;
 ...
 * ===================================================================== */
/* One row of the banner: 19 blank bytes, an edge, twelve translated cells,
 * an edge, and 19 blank again - then the whole screen scrolls up. */
static void banner_row(const uint8_t *cells)
{
    uint16_t di = cga_at(0, 111);
...
static int32_t bonus_end_level_run(void)
{
    ...
    /* The wall closing in: 112 passes of a 26-word band scrolled up six rows
     * with a fresh cap laid on each. */
    uint16_t bp = cga_at(0, 5);
    global.paddle_morphing = 0xff;
    for (uint16_t dh = 112; dh > 0; dh--) {
         ...
         * into the row above, and stepping DI by cga_prev_row(cga_next_row(di)),
    ...
    for (uint16_t i = 0; i < 26; i++)
        vram_setw(cga_at(0, 111) + i * 2, global.results_rows[0][i]);
    screen_scroll_up();
    for (uint16_t i = 0; i < 26; i++)
        vram_setw(cga_at(0, 111) + i * 2, global.results_rows[1][i]);
    screen_scroll_up();
    banner_blank();
     ...
     * walks **up** the level by 12 a time, not down: the loop's `pop si` at
     * 1ac2:43e7 takes the value after the second row's lodsb.
     *
     * `mov si,[0x13ca] / add si, 0xb8` at 1ac2:4337, and that 0xb8 is a whole
     * record plus the cells' own offset - so it is the **next** record's
     * cells, which is the level about to be played. This banner runs inside
     * bonus_end_level_run, and the only place level_src_ptr advances is
     * `level_done` in play_loop, which the longjmp does not reach until this
     * has returned - so the wall closing in is a preview of what is next. */
    const level_t *next =
        (const level_t *)assets_ptr(global.level_src_ptr) + 1;
    const uint8_t *row = next->cells;
    for (uint16_t n = 14; n > 0; n--, row += BRICK_COLS) {
        banner_row(row);
        banner_row(row);
        banner_blank();
    }
    ...
    for (uint16_t r = 0; r < 7; r++) {
        for (uint16_t i = 0; i < 26; i++)
            vram_setw(cga_at(0, 111) + i * 2, global.results_rows[2 + r][i]);
        screen_scroll_up();
    }
    ...
    for (uint16_t pass = 8; pass > 0; pass--) {
        ...
        for (uint16_t row = 4; row > 0; row--) {
            ...
            g_vram[(di + 2) & (CGA_SIZE - 1)] &= (mask_r >> 8);
            g_vram[(di + 3) & (CGA_SIZE - 1)] &= mask_r;
            di = cga_next_row(di);
        }
        input_and_draw_paddle();
        for (uint16_t i = 0; i < 40; i++)
    ...
    for (;;) {
        input_and_draw_paddle();
        ball_step(&global.balls[0]);
        ball_redraw(&global.balls[0]);
 ...
 * +22, six words of level state at +0xc6, and then, at +0xd2, a **count of
...
int32_t next_player(const char *dir)
{
    ...
    memcpy(p->state, global.cell_bitmap.animated_ptr, sizeof p->state);
    ...
    do {
        global.cur_player = 
            ((global.cur_player + 1) % global.player_count);
        q = &global.players[global.cur_player];
    } while (q->lives == 0);
    ...
    memcpy(global.cell_bitmap.animated_ptr, q->state, sizeof q->state);

    for (uint16_t k = 0; k < q->ent_count; k++)
...
void screen_results(const char *dir)
{
    if (global.player_count == 1) {
        ...
        memcpy(global.scratch2.hsc_scratch[0].name, global.players[0].name,
               sizeof global.players[0].name);
        memcpy(global.scratch2.hsc_scratch[0].score, global.players[0].score,
               sizeof global.players[0].score);
        results_finish(dir);
        return;
    }
    ...
    hsc_entry_t *scratch = global.scratch2.hsc_scratch;
    const player_t *q = &global.players[0];
    memcpy(scratch[0].name, q->name, sizeof q->name);
    memcpy(scratch[0].score, q->score, sizeof q->score);
    for (uint16_t k = 1; k < global.player_count; k++) {
        q = &global.players[k];
        uint16_t at = k;
    ...
    for (uint16_t i = 0; i < HSC_WIDTH; i++)       /* 1ac2:0f08 */
        vram_setw(cga_at(HSC_X, HSC_TOP) + i * 2, 0xaaaa);

    uint16_t d = cga_at(HSC_X, HSC_TOP + 1);
    d = draw_run(' ', HSC_WIDTH, d);              /* 1ac2:0f1a */
    ...
    /* 1ac2:1006 - the rest of the panel, a row at a time until it steps off
     * the screen. The screen's last line is 199, so cga_at(WALL_LEFT, CGA_H)
     * is the row after it: a position no row has, which is what lets the
     * original stop on an exact compare rather than a `jae`. */
    while (d != cga_at(WALL_LEFT, CGA_H)) {
        for (uint16_t i = 0; i < 24; i++)
            vram_setw(d + i * 2, 0xaaaa);
        d = cga_next_row(d);
    }
...
/* 1ac2:1a6f  anim_step
 *
 * One step of the level's brick animation. Inside the play loop rather than a
 * routine of its own, which is why verify.c cannot dispatch it: 0x1a6f is an
 * inline block, not an entry.
 *
 * [0x3134] counts down to the next step and reloads from [0x3135]; [0x3136]
 * walks the script, two bytes a step. A 0xffff there is not the end but a
 * jump - the word after it is where to carry on - so an animation loops
 * without being copied, which is what the six scripts in animations_t all do.
 *
 * It was called demo_input_step, from sitting beside the input call at
 * 1ac2:1a4f. It has nothing to do with input or with the demo: 1ac2:1a84
 * compares through **ES**, which is the animations segment here, and every
 * byte it touches is animation state.
 */
void anim_step(void)
{
    if (--global.anim_count != 0)
        return;
 ...
 * The high byte becomes the y, added to BONUS_HOMING_Y + 1 - which is the
 * row bonus_move_down's own `inc al` left the capsule on when it handed
 * over, so the script starts from where the wandering stopped.
 *
 * The x is clamped to WALL_LEFT..WALL_RIGHT - CAPSULE_SIZE, the same walls
 * everything else uses.
 */
int32_t bonus_script(ent_anim_t *b, uint8_t *px, uint8_t *py)
{
    uint16_t step_ptr = b->script_ptr;
    b->script_ptr = step_ptr + 2;
    const bonus_step_t *step = (const bonus_step_t *)global_ptr(step_ptr);
    /* arg.move.steps is the x bonus_move_down saved when it handed over, and
     * nothing writes it again while the script runs - so it is the anchor
     * both halves are measured from. */
    uint8_t anchor = b->arg.move.steps;

    /* One add, at 1ac2:3c4e: the positive path jumps straight to it with
     * `jae 0x3c4d` and the leftward path falls into it past the compare, so
     * only a step that would cross the wall goes anywhere else. */
    uint8_t x;
    if (step->dx < 0 && anchor < (uint8_t)-step->dx)
        x = WALL_LEFT;                  /* it would go through the wall */
    else
        x = anchor + step->dx;
    if (x > WALL_RIGHT - CAPSULE_SIZE)
        x = WALL_RIGHT - CAPSULE_SIZE;
    if (x < WALL_LEFT)
        x = WALL_LEFT;

    *px = x;
    *py = (BONUS_HOMING_Y + 1) + step->dy;   /* `mov al,0x79 / add al,ah` */

    /* The step's dy comes back in AH, and entity_bonus tests `cmp ah, 0xff`
     * to decide whether to commit the move at all - -1 means "stay where you
     * are". Always answering "moved" put the capsule at 121 - 1 = 120, which
     * is what diverged 21,332 frames into level 2: the port walked it to y
     * 120 while the original left it alone. */
    return step->dy != -1;
}
 ...
 * the panel at PANEL_M_X, which `play_frame` drains one cell at a time -
...
void bonus_stop_monsters(void)
{
    ...
    global.extra_timer = 400;
    fill_column(cga_at(PANEL_M_X, PANEL_BAR_Y), 0xaaaa);
    global.extra_pos = cga_at(PANEL_M_X, PANEL_BAR_Y);
}
...
void input_demo(void)
{
    if (io_key_ready()) {
        ...
        if ((key >> 8) == KEY_F10) {
            employee_enter();                   /* 1ac2:4ae0 */
            while ((io_get_key() >> 8) == KEY_F10)
                ;
            screen_restore();                   /* 1ac2:4b4f */
        } else if ((key >> 8) == KEY_F9) {      /* sound */
            runtime.sound_on ^= 1;
    ...
    uint8_t chasing = runtime.demo_ball;
    if (chasing != 0xff) {
        ball_t *b = &global.balls[chasing];
        if (b->y >= DEMO_CHASE_Y &&
            b->dir_y != 1 &&
            b->state != 0) {
            /* Hold the action key down while the laser is armed, so the
             * demo fires as well as chases. */
            global.key_action = global.laser_on != 0;

            uint8_t ball_x = b->x;
            uint8_t paddle = global.paddle_x;
            if (ball_x < paddle) {
                if (paddle != global.paddle_min)
                    global.paddle_x--;
                return;
            }
            /* 1ac2:180b is `add bl,[0x2d3a]`, so the right-hand edge is
             * BL. The paddle stops at paddle_max, which is the wall less its
             * own width, so it never wraps - this is the register. */
            uint8_t right = paddle + global.paddle_width;
            if (ball_x < right)
                return;                         /* already under it */
            if (paddle != global.paddle_max)
                global.paddle_x++;
            return;
        }
    }
    ...
    for (uint8_t cl = 0; cl < 3; cl++) {
        ball_t *b = &global.balls[cl];
        if (b->y > DEMO_CHASE_Y &&
            b->state != 0 &&
            b->dir_y != 1) {
            runtime.demo_ball = cl;
            demo_clamp();
            return;
        }
    }
...
int32_t cheat_sequence(char key)
{
    ...
    if (al != at[0]) {
        ...
        if (al == runtime.cheat_last)
            return 1;
        runtime.cheat_cursor_ptr = runtime_off(runtime.cheat_keys);
        runtime.cheat_last = 0;
        return 0;
    }

    runtime.cheat_last = al;
    runtime.cheat_cursor_ptr++;
    if (at[1] != 0xaa)
        return 1;                       /* more to go */
...
{
    if (global.anim_rate != global.anim_count)
        return;
    const anim_sprite_t *group = (const anim_sprite_t *)
        animations_ptr(animations_w(global.anim_ptr));
    draw_anim_cell(&group[br->piece], br->x, br->y);
}
...
void brick_animated(hit_t *hit, ball_t *ball)
{
    ...
    *cell = now;
     ...
     *
     * Two indirections, and both are pointers: script_ptr is where this
     * level's script begins, and the word **at** it is that script's first
     * entry, which is a group of six frames. So a freshly-hit brick starts at
     * the animation's first group rather than wherever anim_ptr has walked
     * to, and the piece subscripts that group. What the slot keeps is the
     * offset a draw will resolve later - the one place a sprite's address is
     * kept the way the game keeps it rather than followed, which is what
     * animations_off is for. The original's `shl dx, 5` at 1ac2:2d1d and its
     * `add ax, dx` at 1ac2:2d21 are the subscript and the narrowing. */
    const anim_sprite_t *opening = (const anim_sprite_t *)animations_ptr(
        animations_w(animations.level[global.level_number].script_ptr));
    global.cell_bitmap.animated_ptr[now - 24] = animations_off(&opening[piece]);

    /* Draw it once where the brick was, from the script's **current** entry -
     * which is not `first`: the slot above gets the animation's opening group
     * so the brick has somewhere to start, and this frame shows whatever the
     * other animated bricks are already showing. */
    const anim_sprite_t *current = (const anim_sprite_t *)
        animations_ptr(animations_w(global.anim_ptr));
    draw_anim_cell(&current[piece], hit->centre.x, hit->centre.y);

    entity_t *e = entity_alloc();
    e->handler_fn = ENTITY_ANIM_BRICK_FN;
    ent_brick_t *br = &e->p.brick;
    br->x = hit->centre.x;   /* the centre, one word in the original */
    br->y = hit->centre.y;
    br->piece = piece;
    global.level.bricks--;
}
