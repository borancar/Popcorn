...
| `check_pointers.py` | the game keeps **16-bit pointers in its own data** - the chain's links, a cell parked in a slot, an animation cursor - and they go in through `img_off` and come out through `img_ptr`/`img_w`. This parses `game.c` with a C grammar and checks that: a field the game uses as a pointer is named `*_ptr`, a store into one goes through `img_off`, a read goes through a `_ptr` accessor, and what an accessor is handed is **simple**. That last rule is the one with teeth: `img_w(table + index * 2)` is pointer arithmetic without a pointer type, and the C that means it is `img_w(table[index])` with `table` typed. It also lists the fields already feeding an accessor without the suffix, which is the rename written down. Regex cannot do this - telling an argument from the expression inside it is the whole job |
