...
void read_speed_setting(uint32_t speed)
{
    ...
    if (speed == 0)
        speed = 0x6f;
    runtime.delay_count = (uint16_t)( speed - 1);
}
...
void score_add(void)
{
    ...
    uint32_t thresh = global.extra_at;
...
void bonus_update(ent_sprite_t *s, uint16_t nx, uint16_t ny)
{
    ...
    if (y <= 190 && ((y + 15) & 0xff) >= 184) {
        uint32_t bxx = s->x, px = global.paddle_x;
        if (((bxx + 0x0f) & 0xff) >= px &&
...
int32_t ball_on_paddle(ball_t *b)
{
    ...
    if (global.hold_timer == HOLD_RESET) {
        ...
        b->y = PADDLE_TOP;
        b->state = 2;                 /* held */
        global.hold_timer = (uint16_t)((global.hold_timer - global.speed_limit) & 0xffff);
...
static uint16_t panel_row(uint16_t di, uint32_t lead, uint32_t mid,
...
void menu_banner_tick(void)
{
    if (global.banner_state == 2) {
        ...
        uint32_t c = *global_ptr(global.banner_ptr);
        c = ((c ^ 0xaa) - 0x20) & 0xff;
...
uint16_t particle_init(particle_t *p, uint16_t ax_in)
{
    p->x0 = 0x68;
    p->y0 = 0xa0;
    uint16_t ax = (particle_random(ax_in, io_ticks(), 6) + 8) & 0xffff;
    p->speed = (uint16_t)ax;
 ...
 * ===================================================================== */
static uint16_t pillar_pair(uint16_t di, uint32_t v)
{
    vram_setw(di, v);
    vram_setw(di + 50, v);
    return cga_next_row(di);
...
uint16_t ending_particle_init(particle_t *p, uint16_t ax_in)
{
    p->x0 = 0x68;
    p->y0 = 0x98;
    uint16_t ax = (particle_random(ax_in, io_ticks(), 3) + 8) & 0xffff;
    p->speed = (uint16_t)ax;
    ax = (particle_random(ax, io_ticks(), 70) - 35) & 0xffff;
    if (ax == 0)
        ax = 10;
